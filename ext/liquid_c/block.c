...
static ID intern_raise_missing_variable_terminator,
    intern_raise_missing_tag_terminator, intern_is_blank, intern_parse,
...
static void block_body_free(void *ptr) {
  ...
  if (!body->compiled && body->as.intermediate.code) {
    // Free the assembler instead of recycling it because the vm_assembler_pool
    // may have been GC'd
...
static size_t block_body_memsize(const void *ptr) {
  ...
  if (body->compiled) {
    ...
    return sizeof(block_body_t) +
           vm_assembler_alloc_memsize(body->as.intermediate.code);
  }
}

const rb_data_type_t block_body_data_type = {"liquid_block_body",
                                             {
                                                 block_body_mark,
                                                 block_body_free,
                                                 block_body_memsize,
...
#define BlockBody_Get_Struct(obj, sval)                                        \
  TypedData_Get_Struct(obj, block_body_t, &block_body_data_type, sval)

static VALUE block_body_allocate(VALUE klass) {
  block_body_t *body;
  VALUE obj =
      TypedData_Make_Struct(klass, block_body_t, &block_body_data_type, body);
...
static VALUE block_body_initialize(VALUE self, VALUE parse_context) {
  ...
  body->as.intermediate.vm_assembler_pool =
      parse_context_get_vm_assembler_pool(parse_context);
  body->as.intermediate.code = vm_assembler_pool_alloc_assembler(
      body->as.intermediate.vm_assembler_pool);
...
static int is_id(int c) { return rb_isalnum(c) || c == '_'; }
...
static tag_markup_t internal_block_body_parse(block_body_t *body,
  ...
  while (true) {
    ...
    if (token_start_line_number != 0) {
      rb_ivar_set(parse_context->ruby_obj, id_ivar_line_number,
                  UINT2NUM(token_start_line_number));
    }
    ...
    case TOKEN_INVALID: {
      ...
      if (token.str_full[1] == '%')
        raise_method_id = intern_raise_missing_tag_terminator;

      rb_funcall(cLiquidBlockBody, raise_method_id, 2, str,
                 parse_context->ruby_obj);
      goto loop_break;
    }
    case TOKEN_RAW: {
      const char *start = token.str_full,
                 *end = token.str_full + token.len_full;
      ...
      vm_assembler_add_write_raw(body->as.intermediate.code, token_start,
    ...
    case TOKEN_TAG: {
      const char *start = token.str_trimmed,
                 *end = token.str_trimmed + token.len_trimmed;
      ...
      if (name_len == 0) {
        if (name_start < end && *name_start == '#') { // inline comment
          ...
          VALUE str = rb_enc_str_new(token.str_trimmed, token.len_trimmed,
                                     utf8_encoding);
      ...
      if (name_len == 6 && strncmp(name_start, "liquid", 6) == 0) {
        ...
        tokenizer_setup_for_liquid_tag(tokenizer, markup_start, end,
                                       line_number);
        ...
        *tokenizer = saved_tokenizer;
        if (unknown_tag.name != Qnil) {
          rb_funcall(cLiquidBlockBody, intern_unknown_tag_in_liquid_tag, 2,
                     unknown_tag.name, parse_context->ruby_obj);
          goto loop_break;
        }
        break;
      }
      ...
      VALUE tag_name =
          rb_enc_str_new(name_start, name_end - name_start, utf8_encoding);
      VALUE tag_class =
          rb_funcall(tag_registry, intern_square_brackets, 1, tag_name);

      const char *markup_start = read_while(name_end, end, rb_isspace);
      VALUE markup =
          rb_enc_str_new(markup_start, end - markup_start, utf8_encoding);
      ...
      VALUE new_tag =
          rb_funcall(tag_class, intern_parse, 4, tag_name, markup,
                     parse_context->tokenizer_obj, parse_context->ruby_obj);

      if (body->as.intermediate.blank &&
          !RTEST(rb_funcall(new_tag, intern_is_blank, 0)))
        body->as.intermediate.blank = false;

      if (tokenizer->raw_tag_body) {
        if (tokenizer->raw_tag_body_len) {
          vm_assembler_add_write_raw(body->as.intermediate.code,
                                     tokenizer->raw_tag_body,
...
static void ensure_intermediate_not_parsing(block_body_t *body) {
  ...
  if (body->as.intermediate.code->parsing) {
    rb_raise(rb_eRuntimeError,
             "Liquid::C::BlockBody is in a incompletely parsed state");
  }
}

static VALUE block_body_parse(VALUE self, VALUE tokenizer_obj,
                              VALUE parse_context_obj) {
  parse_context_t parse_context = {
  ...
  if (body->as.intermediate.parse_context != parse_context_obj) {
    rb_raise(rb_eArgError,
             "Liquid::C::BlockBody#parse called with different parse context");
  }
...
static VALUE block_body_freeze(VALUE self) {
  ...
  body->as.compiled.document_body_entry =
      document_body_write_block_body(document_body, blank, render_score, code);
...
static VALUE block_body_render_to_output_buffer(VALUE self, VALUE context,
                                                VALUE output) {
  ...
  liquid_vm_render(document_body_get_block_body_header_ptr(entry),
                   document_body_get_constants_ptr(entry), context, output);
  return output;
}
...
static VALUE block_body_blank_p(VALUE self) {
  ...
  BlockBody_Get_Struct(self, body);
  if (body->compiled) {
    block_body_header_t *body_header = document_body_get_block_body_header_ptr(
        &body->as.compiled.document_body_entry);
...
static VALUE block_body_remove_blank_strings(VALUE self) {
  ...
  if (!body->as.intermediate.blank) {
    rb_raise(
        rb_eRuntimeError,
        "remove_blank_strings only support being called on a blank block body");
  }
...
static void memoize_variable_placeholder(void) {
  if (variable_placeholder == Qnil) {
    VALUE cLiquidCVariablePlaceholder =
        rb_const_get(mLiquidC, rb_intern("VariablePlaceholder"));
    variable_placeholder =
        rb_class_new_instance(0, NULL, cLiquidCVariablePlaceholder);
  }
}
...
static VALUE block_body_nodelist(VALUE self) {
  ...
  block_body_header_t *body_header =
      document_body_get_block_body_header_ptr(entry);
...
static VALUE block_body_disassemble(VALUE self) {
  ...
  return vm_assembler_disassemble(
      start_ip, start_ip + header->instructions_bytes, &entry->body->constants);
}
...
static VALUE block_body_add_evaluate_expression(VALUE self, VALUE expression) {
  ...
  BlockBody_Get_Struct(self, body);
  ensure_intermediate(body);
  vm_assembler_add_evaluate_expression_from_ruby(body->as.intermediate.code,
                                                 self, expression);
  return self;
}
...
static VALUE block_body_add_find_variable(VALUE self, VALUE expression) {
  ...
  BlockBody_Get_Struct(self, body);
  ensure_intermediate(body);
  vm_assembler_add_find_variable_from_ruby(body->as.intermediate.code, self,
                                           expression);
  return self;
}
...
static VALUE block_body_add_lookup_key(VALUE self, VALUE expression) {
  ...
  BlockBody_Get_Struct(self, body);
  ensure_intermediate(body);
  vm_assembler_add_lookup_key_from_ruby(body->as.intermediate.code, self,
                                        expression);
  return self;
}
...
static VALUE block_body_add_filter(VALUE self, VALUE filter_name,
                                   VALUE num_args) {
  block_body_t *body;
  BlockBody_Get_Struct(self, body);
  ensure_intermediate(body);
  vm_assembler_add_filter_from_ruby(body->as.intermediate.code, filter_name,
                                    num_args);
  return self;
}
...
void liquid_define_block_body(void) {
  intern_raise_missing_variable_terminator =
      rb_intern("raise_missing_variable_terminator");
  intern_raise_missing_tag_terminator =
      rb_intern("raise_missing_tag_terminator");
  ...
  VALUE cLiquidCBlockBody =
      rb_define_class_under(mLiquidC, "BlockBody", rb_cObject);
  ...
  rb_define_method(cLiquidCBlockBody, "render_to_output_buffer",
                   block_body_render_to_output_buffer, 2);
  rb_define_method(cLiquidCBlockBody, "remove_blank_strings",
                   block_body_remove_blank_strings, 0);
  ...
  rb_define_method(cLiquidCBlockBody, "add_evaluate_expression",
                   block_body_add_evaluate_expression, 1);
  rb_define_method(cLiquidCBlockBody, "add_find_variable",
                   block_body_add_find_variable, 1);
  rb_define_method(cLiquidCBlockBody, "add_lookup_command",
                   block_body_add_lookup_command, 1);
  rb_define_method(cLiquidCBlockBody, "add_lookup_key",
                   block_body_add_lookup_key, 1);
  rb_define_method(cLiquidCBlockBody, "add_new_int_range",
                   block_body_add_new_int_range, 0);

  rb_define_method(cLiquidCBlockBody, "add_hash_new", block_body_add_hash_new,
                   1);
