...
static void document_body_write_tag_markup(document_body_t *body, VALUE tag_markup_obj)
{
    ...
    size_t tag_markup_offset = c_buffer_size(&body->as.mutable.buffer);
    c_buffer_extend_for_write(&body->as.mutable.buffer, sizeof(tag_markup_header_t));
    ...
    header.total_len = (uint32_t)(c_buffer_size(&body->as.mutable.buffer) - tag_markup_offset);

    memcpy(body->as.mutable.buffer.data + tag_markup_offset, &header, sizeof(tag_markup_header_t));
}
