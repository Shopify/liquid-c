...
static ID idNodeList, idClear, idBlank, idBlankQ, idTags,
          idParse, idSquareBrackets;
...
VALUE rb_block_parse(VALUE self, VALUE tokens, VALUE options)
{
    ...
    while (tokenizer_next(tokenizer, &token)) {
        if (token.type == TOKEN_INVALID) {
            ...
            ID raise_method_id = token.str[1] == '%' ? rb_intern("raise_missing_variable_terminator") : rb_intern("raise_missing_tag_terminator");
        ...
        if (token.type == TOKEN_RAW) {
            ...
            if (tag != Qnil) {
                ...
                return rb_yield_values(2, name, markup);
            }
        }
    }
